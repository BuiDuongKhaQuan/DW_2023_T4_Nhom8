...
public class LoadDBStagingToDW {
    ...
    public static void loadDW() throws SQLException {
        ...
        try ( // Establish connections to control and staging databases using JDBC
            ...
            // Insert log
            log(1, "Load StagingToDW Success");
    ...
    public static void log(int id_conf, String status) throws SQLException {
        try (Connection connectionControl = DriverManager.getConnection(urlControl, userName, password);
             PreparedStatement preparedStatementLog = connectionControl.prepareStatement("INSERT INTO log (id_config, time_extract, id_date, status_extract) VALUES (?, CURRENT_TIME(), CURDATE(), ?)")) {
            preparedStatementLog.setInt(1, id_conf);
            preparedStatementLog.setString(2, status);
